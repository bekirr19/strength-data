    ...
    <div style={{ position:'relative',height:'100%',display:'flex',flexDirection:'column' }}>
      <div style={{ flex:1,display:'flex',alignItems:'center',justifyContent:'center' }}>
        <EmptyState tone="accent" icon={<LIcon name="Dumbbell" size={28} />}
          ...
          action={<DS_ST.Button variant="primary" icon={<LIcon name="Plus" size={18} />} onClick={() => nav.go('today')}>Add manually</DS_ST.Button>} />
      </div>
      <div style={{ padding:'0 16px 20px' }}>
        <DS_ST.BottomNav activeKey="today" items={[{key:'today',label:'Today',icon:<LIcon name="CalendarDays" size={20}/>},{key:'exercises',label:'Exercises',icon:<LIcon name="ListChecks" size={20}/>}]}
  ...
  return (
    <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 20px',gap:24,minHeight:320 }}>
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:10,alignItems:'center' }}>
        ...
        <span style={{ fontSize:'var(--text-xs)',color:'var(--text-tertiary)',fontWeight:600 }}>Bench Press · Mon 15 Jun</span>
      </div>
      {isNew && <PRConfetti />}
    </div>
  );
